...
(function() {
    ...
    //reset everything
        ...
        dy = 0;
        hitWall = false;
        if($('a[data-toggle="tab"].active').attr("href")!="#games"){
            document.removeEventListener("keydown", restartSnake);
            return
        }
...
//                    rest = true;
            ...
            else if (score <= 2000 && score % 125 == 0 && 1000 < score){
                rest = false;
                growthspeed = 0;
                SHORTEN_LENGTH += 1;
            }
            if (timer == 0 && !rest) {
                snake.splice(-SHORTEN_LENGTH,SHORTEN_LENGTH);
                timer = growthspeed;
            }
            else if (timer >0){
                timer -= 1;
     ...
     */
    function drawFood(coordinates) {
        ctx.fillStyle = theme6;
        ctx.strokeStyle = theme1;
        ctx.fillRect(coordinates[0], coordinates[1], BLOCK_SIZE, BLOCK_SIZE);
        ctx.strokeRect(coordinates[0], coordinates[1], BLOCK_SIZE, BLOCK_SIZE);
    }
    /**
    ...
    function advanceSnake() {
        ...
        if(isE){
            for (var i = 0; i < snake.length; i++){
                for (o = 0; o < foodXY.length; o++){
                    if(snake[i].x == foodXY[o][0] && snake[i].y == foodXY[o][1]){
                        score += 1;
                        createFood();
                    }
                }
     ...
     */
     ...
     function didEatFood(){
        for (i = 0; i < foodXY.length; i++){
            if(foodXY[i][0] == snake[0].x && foodXY[i][1] == snake[0].y){
                foodXY.splice(i, 1);
                return true
            }
        }
    ...
    function createFood() {
        ...
        foodX = randomTen(-BLOCK_SIZE, gameCanvas.width - BLOCK_SIZE);
        // Generate a random number for the food y-coordinate
        foodY = randomTen(-BLOCK_SIZE, gameCanvas.height - BLOCK_SIZE);
        ...
        for (i=0; i < foodXY.length; i++){
            if(foodXY[i][0] == foodX && foodXY[i][1] == foodY){
                foodIsValid = false;
            }
        }
        if(foodIsValid){
            foodXY.push([foodX,foodY]);
        }
        else{
            createFood();
        }
